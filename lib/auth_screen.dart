  ...
  void _submit() async {
    ...
    try {
      if (isLogin) {
        ...
        // Capture the result of the registration attempt
        int result = await DatabaseHelper.instance.registerUser(user, pass);
        if (!mounted) return;

        // Check if the database returned our -1 error code
        if (result == -1) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          ...
          return; // The app will stop here Do not create the account or switch screens.
        }
